...
public enum TrimModifierModel implements IBakedModifierModel {
  ...
  static {
    for (ArmorItem.Type type : ArmorItem.Type.values()) {
      ...
      // BODY type (for animal armor) doesn't have a trim texture, use empty
      String trimPath = switch (type) {
        case HELMET, CHESTPLATE, LEGGINGS, BOOTS -> "trims/items/" + type.getName() + "_trim";
        default -> "trims/items/helmet_trim"; // fallback for unknown types
      };
      TRIM_TEXTURES[type.ordinal()] = ResourceLocation.withDefaultNamespace(trimPath);
    }
  }
          ...
          if (level != null) {
            ...
            if (material != null) {
              ...
              if (MissingTextureAtlasSprite.getLocation().equals(sprite.contents().name())) {
                ...
                TConstruct.LOG.error("Missing material specific texture {}, defaulting to tinting base texture #{}", path, toHex(color));
              }
