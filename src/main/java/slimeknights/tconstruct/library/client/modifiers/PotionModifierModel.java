  ...
  public void addQuads(IToolStackView tool, ModifierEntry modifier, Function<Material,TextureAtlasSprite> spriteGetter, Transformation transforms, boolean isLarge, int startTintIndex, Consumer<Collection<BakedQuad>> quadConsumer, @Nullable ItemLayerPixels pixels) {
    ...
    if (texture != null) {
      ...
      if (toolData.contains(key, Tag.TAG_STRING)) {
        ...
        if (id != null) {
          Holder<Potion> potionHolder = BuiltInRegistries.POTION.getHolder(id).orElse(null);
          if (potionHolder != null && potionHolder.isBound()) {
            int color = PotionContents.getColor(potionHolder.value().getEffects());
