...
import slimeknights.tconstruct.tools.modules.SmashingModule;
...
public class ModifierModelManager implements IEarlySafeManagerReloadListener {
  ...
  static {
    registerBuiltin(TConstruct.getResource("normal"), NormalModifierModel.UNBAKED_INSTANCE);
    registerBuiltin(TConstruct.getResource("overslime"), OverslimeModifierModel.UNBAKED_INSTANCE);
    registerBuiltin(TConstruct.getResource("fluid"), FluidModifierModel.UNBAKED_INSTANCE);
    registerBuiltin(TConstruct.getResource("tank"), TankModifierModel.UNBAKED_INSTANCE);
    registerBuiltin(TConstruct.getResource("material"), MaterialModifierModel.UNBAKED_INSTANCE);
    registerBuiltin(TConstruct.getResource("dyed"), DyedModifierModel.UNBAKED_INSTANCE);
    registerBuiltin(TConstruct.getResource("trim"), TrimModifierModel.UNBAKED_INSTANCE);
    registerBuiltin(TConstruct.getResource("potion"), PotionModifierModel.UNBAKED_INSTANCE);
    registerBuiltin(TConstruct.getResource("smashing_fluid"), new FluidModifierModel.Unbaked(SmashingModule.TANK_HELPER));
  }
  ...
  private static void registerBuiltin(ResourceLocation name, IUnbakedModifierModel model) {
  ...
  public void onReloadSafe(ResourceManager manager) {
    ...
    if (!eventFired) {
      net.neoforged.fml.ModLoader.postEvent(new ModifierModelRegistrationEvent());
      eventFired = true;
    }
  ...
  public static Map<ModifierId,IBakedModifierModel> getModelsForTool(Function<Material,TextureAtlasSprite> spriteGetter, List<ResourceLocation> smallModifierRoots, List<ResourceLocation> largeModifierRoots) {
    ...
    for (Entry<ModifierId, IUnbakedModifierModel> entry : modifierModels.entrySet()) {
      ...
      ResourceLocation idLocation = id.asResourceLocation();
      IUnbakedModifierModel model = entry.getValue();
      IBakedModifierModel toolModel = model.forTool(
        name -> getTexture(smallModifierRoots, validator, idLocation, name),
        name -> getTexture(largeModifierRoots, validator, idLocation, name));
      if (toolModel != null) {
