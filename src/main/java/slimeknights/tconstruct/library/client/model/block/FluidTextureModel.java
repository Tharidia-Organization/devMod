...
public class FluidTextureModel implements IUnbakedGeometry<FluidTextureModel> {
  ...
  private static final ResourceLocation BAKE_LOCATION = TConstruct.getResource("fluid_texture_dynamic");
  ...
  private static class Baked extends DynamicBakedWrapper<BakedModel> {
    ...
    private BakedModel getRetexturedModel(BakedCacheKey key) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        if (fluidParts.get(i)) {
          ColoredBlockModel.bakePart(builder, textured, element, luminosity, spriteGetter, transform, fluidTransformer, colors.isUvLock(defaultUvLock), TankModel.BAKE_LOCATION);
        } else {
          ...
          ColoredBlockModel.bakePart(builder, textured, element, colors.luminosity(), spriteGetter, transform, partTransformer, colors.isUvLock(defaultUvLock), TankModel.BAKE_LOCATION);
        }
      }
