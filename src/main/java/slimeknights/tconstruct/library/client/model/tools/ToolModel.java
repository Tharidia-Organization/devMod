...
public class ToolModel implements IUnbakedGeometry<ToolModel> {
  ...
  private static List<FirstModifier> parseFirstModifiers(JsonObject json) {
    ...
    for (int i = 0; i < array.size(); i++) {
      ...
      if (element.isJsonPrimitive()) {
        ...
        if (id == null) {
          throw new JsonSyntaxException("Invalid modifier id in first_modifiers[" + i + "]");
        }
        list.add(new FirstModifier(id, false));
        ...
        boolean forced = GsonHelper.getAsBoolean(object, "forced", false);
  ...
  public static ToolModel deserialize(JsonObject json, JsonDeserializationContext context) {
    ...
    if (json.has("modifier_roots")) {
      ...
      if (isLarge) {
        ...
        smallModifierRoots = JsonHelper.parseList(modifierRoots, "small", JsonHelper::convertToResourceLocation);
        largeModifierRoots = JsonHelper.parseList(modifierRoots, "large", JsonHelper::convertToResourceLocation);
      } else {
        // small requires an array
        smallModifierRoots = JsonHelper.parseList(json, "modifier_roots", JsonHelper::convertToResourceLocation);
      }
    }
  ...
  private ToolModel(List<ToolPart> toolParts, boolean isLarge, Vec2 offset, List<ResourceLocation> smallModifierRoots,
                    List<ResourceLocation> largeModifierRoots, List<FirstModifier> firstModifiers, @Nullable ResourceLocation ammoKey,
                    boolean flipAmmo, boolean leftAmmo, Vec2 smallAmmoOffset, Vec2 largeAmmoOffset, boolean showTraits) {
  ...
  public static final class MaterialOverrideHandler extends NestedOverrides {
    ...
    public BakedModel resolve(BakedModel originalModel, ItemStack stack, @Nullable ClientLevel world, @Nullable LivingEntity entity, int seed) {
      ...
      if (ammoKey != null && persistentData.contains(ammoKey, Tag.TAG_COMPOUND)) {
        // In 1.21, ItemStack.of() is replaced with parseOptional()
        // We need a proper registry lookup to deserialize items correctly
        net.minecraft.core.HolderLookup.Provider registries = null;
        if (world != null) {
          registries = world.registryAccess();
        } else {
          // Store in local variable to avoid potential null between check and use
        ...
        if (registries != null) {
          ...
          net.minecraft.world.item.component.CustomData customData = ammo.get(net.minecraft.core.component.DataComponents.CUSTOM_DATA);
          if (customData != null) {
            builder.add(customData.copyTag());
          }
        } else {
          // Fallback when no registry is available - shouldn't normally happen during rendering
