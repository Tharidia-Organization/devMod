...
public class TelepadPortalRenderer implements BlockEntityRenderer<TelepadBlockEntity> {
    // Portal dimensions (30% larger)
    private static final float PORTAL_WIDTH = 1.82f;
    private static final float PORTAL_HEIGHT = 2.6f;
    ...
    public void render(TelepadBlockEntity be, float partialTick, PoseStack poseStack,
        ...
        // Render the portal with all layers
    ...
    private void renderVortexCenter(Matrix4f matrix, float halfWidth, float halfHeight,
        ...
        // Base vortex fill (lighter blue/purple with transparency) - 30% larger
        ...
        // Edge points with gradient - scaled up by 30% (0.92 -> 0.88 to stay inside ring)
        ...
        // Spiral arms - 30% larger (0.85 * 1.3 = ~1.1, capped at 0.95 to stay inside)
    ...
    private void renderSpiralArms(Matrix4f matrix, float halfWidth, float halfHeight,
        ...
        for (int arm = 0; arm < VORTEX_ARMS; arm++) {
            ...
            int spiralSegments = 28;  // More segments for smoother arms
            float armWidth = 0.12f;   // Wider arms (was 0.08f)
            ...
            for (int i = 0; i <= spiralSegments; i++) {
                ...
                float alpha = (1.0f - t * 0.7f) * 0.8f * intensity;  // More visible
                float r = 0.5f + t * 0.3f;   // Brighter
                float g = 0.85f + t * 0.1f;  // Brighter
                float b = 1.0f;
     ...
     * with irregular, energetic appearance
     */
    private void renderEnergyVortex(Matrix4f matrix, float halfWidth, float halfHeight,
        ...
        // Energy trails that extend beyond the oval (scale > 1.0)
        int numTrails = 12;  // More trails for fuller effect
        float extendScale = 1.25f;  // Extend 25% beyond oval
        ...
        for (int trail = 0; trail < numTrails; trail++) {
            ...
            // Add irregularity using pseudo-random based on trail index and time
            ...
            for (int i = 0; i <= segments; i++) {
                ...
                // Spiral extends from center outward beyond the oval
                float spiralAngle = rotation * speedVariation + trailOffset + t * (float) Math.PI * 2.0f;

                // Radius starts small, extends beyond oval boundary
                float baseRadius = t * extendScale;
                // Add wave distortion for irregularity
                ...
                // Perpendicular for trail width (thinner at edges)
                ...
                // Color: bright cyan, fades at edges
                // More visible in the extending region (t > 0.8)
                float edgeFade = t > 0.8f ? (1.0f - (t - 0.8f) * 3.0f) : 1.0f;
                float alpha = (1.0f - t * 0.5f) * 0.6f * intensity * Math.max(edgeFade, 0.2f);

                // Flickering brightness for energy effect
        ...
        // Add energy sparks/particles along the trails
    ...
    private void renderEnergySparks(Matrix4f matrix, float halfWidth, float halfHeight,
        ...
        for (int spark = 0; spark < numSparks; spark++) {
            // Each spark has a unique position along the spiral
            float sparkPhase = (float) (spark * 2.0 * Math.PI / numSparks);
            float sparkRadius = 0.3f + (spark % 8) * 0.12f;  // Distributed at different radii

            // Animate spark position along spiral
            float sparkAngle = rotation * 1.2f + sparkPhase + (float) Math.sin(spark * 0.7f) * 0.5f;

            // Add jitter for irregular movement
    ...
    private void renderEnergyRing(Matrix4f matrix, float halfWidth, float halfHeight,
        ...
        // Core bright ring (innermost, most visible)
