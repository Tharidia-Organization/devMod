...
public record FoundryFluidContainerModel(FluidStack fluid, boolean flipGas) implements IUnbakedGeometry<FoundryFluidContainerModel> {
  public static final IGeometryLoader<FoundryFluidContainerModel> LOADER = FoundryFluidContainerModel::deserialize;
  ...
  public static FoundryFluidContainerModel deserialize(JsonObject json, JsonDeserializationContext context) {
    ...
    return new FoundryFluidContainerModel(fluidStack, flipGas);
  }
  ...
  private static BakedModel bakeInternal(IGeometryBakingContext context, Function<Material, TextureAtlasSprite> spriteGetter, ModelState modelState, ItemOverrides overrides, FluidStack fluid, boolean flipGas) {
    ...
    if (particleSprite == null) {
      DevMod.LOGGER.error("No valid particle sprite for fluid container model, you should supply either 'base' or 'particle'");
    ...
    if (fluidSprite != null) {
      ...
      if (color != -1) {
        FoundryColoredBlockModel.applyColorQuadTransformer(color).processInPlace(quads);
      }
  ...
  private static final ResourceLocation BAKE_LOCATION = ResourceLocation.fromNamespaceAndPath(DevMod.MODID, "fluid_container_dynamic");
