 ...
 * @see ToolModel
 */
public class FoundryMaterialBlockModel implements IUnbakedGeometry<FoundryMaterialBlockModel> {
  /** Location for dynamic baking */
  private static final ResourceLocation BAKE_LOCATION = ResourceLocation.fromNamespaceAndPath(DevMod.MODID, "material_block_dynamic");
  /** Shared loader instance */
  public static final IGeometryLoader<FoundryMaterialBlockModel> LOADER = FoundryMaterialBlockModel::deserialize;
  ...
  private static Set<String> parseStringSet(JsonElement element, String key) {
    ...
    if (element.isJsonArray()) {
      ...
      for (int i = 0; i < array.size(); i++) {
        values.add(GsonHelper.convertToString(array.get(i), key + "[" + i + "]"));
      }
      return Set.copyOf(values);
    }
    if (element.isJsonPrimitive()) {
      return Set.of(GsonHelper.convertToString(element, key));
    }
    throw new IllegalArgumentException("Expected " + key + " to be a string or array");
  }

  private static List<Set<String>> parseParts(JsonObject json) {
    if (!json.has("parts")) {
      return List.of();
    }
    ...
    List<Set<String>> parts = new java.util.ArrayList<>(partsArray.size());
    for (int i = 0; i < partsArray.size(); i++) {
      parts.add(parseStringSet(partsArray.get(i), "parts[" + i + "]"));
    }
    return List.copyOf(parts);
  }

  public FoundryMaterialBlockModel(FoundrySimpleBlockModel model, List<Set<String>> parts, ModelType type) {
    this.model = model;
    this.parts = parts;
    this.type = type;
  }
  ...
  public static FoundryMaterialBlockModel deserialize(JsonObject json, JsonDeserializationContext context) {
    ...
    if (json.has("retextured")) {
      return new FoundryMaterialBlockModel(model, List.of(parseStringSet(json.get("retextured"), "retextured")), ModelType.ANVIL);
    }
    if (json.has("material")) {
      return new FoundryMaterialBlockModel(model, List.of(parseStringSet(json.get("material"), "material")), ModelType.PART);
    }
    return new FoundryMaterialBlockModel(model, parseParts(json), ModelType.TOOL);
  }
  ...
  public BakedModel bake(IGeometryBakingContext owner, ModelBaker baker, Function<Material, TextureAtlasSprite> spriteGetter, ModelState transform, ItemOverrides overrides) {
    ...
    List<Set<String>> parts = this.parts.stream().map(part -> FoundryRetexturedModel.getAllRetextured(owner, model, part)).toList();
  ...
  private static abstract class AbstractBaked<T,P> extends FoundryDynamicBakedWrapper<BakedModel> {
    ...
    private AbstractBaked(BakedModel original, IGeometryBakingContext owner, FoundrySimpleBlockModel model, ModelState transform, boolean particleRetextured, ModelProperty<P> property) {
      super(original);
      this.owner = owner;
      this.model = model;
    ...
    protected void fetchMaterial(MaterialVariantId material, Set<String> retextured, Function<Material, TextureAtlasSprite> spriteGetter, Map<String, Material> replacements, Map<String, TintedSprite> tints) {
      ...
      if (info != null) {
        ...
        for (String name : retextured) {
          ...
          if (tinted == null) {
            tinted = info.getSprite(texture, spriteGetter, material);
    ...
    protected BakedModel bakeWith(T materials) {
      ...
      IGeometryBakingContext retextureContext = new FoundryExtraTextureContext(owner, replacements);
      ...
      SimpleBakedModel.Builder builder = FoundrySimpleBlockModel.bakedBuilder(owner, originalModel.getOverrides()).particle(particle);
      ...
      for (int i = 0; i < size; i++) {
        ...
        if (tint != null) {
          IQuadTransformer partTransformer = tint.color() == -1 ? quadTransformer : quadTransformer.andThen(FoundryColoredBlockModel.applyColorQuadTransformer(tint.color()));
          FoundryColoredBlockModel.bakePart(builder, retextureContext, part, tint.emissivity(), spriteGetter, transform, partTransformer, uvlock, BAKE_LOCATION);
        } else {
          FoundrySimpleBlockModel.bakePart(builder, retextureContext, part, spriteGetter, transform, quadTransformer, BAKE_LOCATION);
        }
      }
  ...
  private static class BakedTool extends AbstractBaked<MaterialIdNBT,MaterialIdNBT> {
    ...
    private final Cache<MaterialIdNBT, BakedModel> cache = CacheBuilder
      ...
      .maximumSize(FoundryMaterialRenderInfoLoader.INSTANCE.getAllRenderInfoCount() * 3L / 2)
      .build();

    private final List<Set<String>> parts;
    private final ItemOverrides overrides;
    private BakedTool(BakedModel original, IGeometryBakingContext owner, FoundrySimpleBlockModel model, ModelState transform, List<Set<String>> parts, boolean particleRetextured) {
      super(original, owner, model, transform, particleRetextured, FoundryModelProperties.MATERIALS);
      this.parts = parts;
    ...
    protected BakedModel getCachedModel(MaterialIdNBT materials) {
      if (!materials.getMaterials().isEmpty()) {
        try {
          ...
          DevMod.LOGGER.error("Failed to get tool model from cache", e);
        }
      }
      return originalModel;
    }
  ...
  private static abstract class BakedSingleMaterial<P> extends AbstractBaked<MaterialVariantId,P> {
    ...
    private BakedSingleMaterial(BakedModel original, IGeometryBakingContext owner, FoundrySimpleBlockModel model, ModelState transform, Set<String> retexture, ModelProperty<P> property) {
  ...
  private static class SimpleBakedTool extends BakedSingleMaterial<MaterialIdNBT> {
    private final ItemOverrides overrides;
    private SimpleBakedTool(BakedModel original, IGeometryBakingContext owner, FoundrySimpleBlockModel model, ModelState transform, Set<String> retexture) {
      super(original, owner, model, transform, retexture, FoundryModelProperties.MATERIALS);
      this.overrides = new MaterialsOverrides(this, original.getOverrides());
    }
  ...
  private static class BakedPart extends BakedSingleMaterial<MaterialVariantId> {
    private final ItemOverrides overrides;
    private BakedPart(BakedModel original, IGeometryBakingContext owner, FoundrySimpleBlockModel model, ModelState transform, Set<String> retexture) {
      super(original, owner, model, transform, retexture, FoundryModelProperties.MATERIAL);
  ...
  private static class BakedAnvil extends BakedSingleMaterial<MaterialVariantId> {
    ...
    private BakedAnvil(BakedModel original, IGeometryBakingContext owner, FoundrySimpleBlockModel model, ModelState transform, Set<String> retexture) {
      super(original, owner, model, transform, retexture, FoundryModelProperties.MATERIAL);
    ...
    /** Rebakes the model with a specific texture. See also {@link FoundryRetexturedModel} */
    ...
    private BakedModel getCachedModel(Block block) {
      return this.blockCache.computeIfAbsent(FoundryModelHelper.getParticleTexture(block), blockBaker);
    }
    ...
    @Override
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, RandomSource rand, ModelData extraData, @Nullable RenderType renderType) {
      Block block = extraData.get(FoundryRetexturedHelper.BLOCK_PROPERTY);
      if (block != null) {
