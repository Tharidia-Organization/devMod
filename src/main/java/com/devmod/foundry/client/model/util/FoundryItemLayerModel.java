 ...
 */
public class FoundryItemLayerModel implements IUnbakedGeometry<FoundryItemLayerModel> {
  /** Model loader instance */
  public static final IGeometryLoader<FoundryItemLayerModel> LOADER = FoundryItemLayerModel::deserialize;
  ...
  public FoundryItemLayerModel(List<LayerData> layers) {
    this.layers = layers;
  }
  ...
  public static RenderTypeGroup getDefaultRenderType(IGeometryBakingContext context) {
    ...
    if (renderTypeHint != null) {
      ...
      // In NeoForge 1.21+, use translucent for both render types
  ...
  public BakedModel bake(IGeometryBakingContext owner, ModelBaker baker, Function<Material,TextureAtlasSprite> spriteGetter, ModelState modelTransform, ItemOverrides overrides) {
    ...
    FoundryReversedListBuilder<QuadGroup> quadBuilder = new FoundryReversedListBuilder<>();
    FoundryItemLayerPixels pixels = textures.size() == 1 ? null : new FoundryItemLayerPixels();
   ...
   */
  public static List<BakedQuad> getQuadsForSprite(int color, int tint, TextureAtlasSprite sprite, Transformation transform, int emissivity, @Nullable FoundryItemLayerPixels pixels) {
    ...
    // setup quad builder - in NeoForge 1.21, QuadBakingVertexConsumer uses no-arg constructor
    // and bakeQuad() is called after each quad's 4 vertices are added
  ...
  /**
   * Gets the quad to display in GUIs for the given sprite. Unlike {@link #getQuadsForSprite(int, int, TextureAtlasSprite, Transformation, int, FoundryItemLayerPixels)}, this method will not build side quads as GUIs don't see those.
  ...
  public static BakedQuad getQuadForGui(int color, int tint, TextureAtlasSprite sprite, Transformation transform, int emissivity) {
    ...
    // Return the last quad which should be the SOUTH (front) face
    for (int i = allQuads.size() - 1; i >= 0; i--) {
      BakedQuad quad = allQuads.get(i);
      if (quad.getDirection() == Direction.SOUTH) {
        return quad;
      }
    }
    // Fallback to last quad if no SOUTH found
    return allQuads.isEmpty() ? null : allQuads.get(allQuads.size() - 1);
  }
  ...
  private static void putVertex(VertexConsumer consumer, Direction side, float x, float y, float z, float u, float v, int color, int luminosity) {
    // format is always DefaultVertexFormat#BLOCK
    // Extract color components from AARRGGBB format
    int a = (color >> 24) & 0xFF;
    int r = (color >> 16) & 0xFF;
    int g = (color >> 8) & 0xFF;
    int b = color & 0xFF;
    int light = (luminosity << 4);
    // Use the 1.21 fluent VertexConsumer API
  ...
  public record LayerData(int color, int luminosity, boolean noTint, @Nullable ResourceLocation renderType) {
    ...
    public static LayerData fromJson(JsonObject json) {
      int color = -1;
      if (json.has("color")) {
        // Color can be specified as hex string (e.g., "FF7F3FB2") or as integer
  ...
  public static FoundryItemLayerModel deserialize(JsonObject json, JsonDeserializationContext context) {
    ...
    return new FoundryItemLayerModel(layers);
  }
}
