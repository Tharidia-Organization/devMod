...
public class GameplayEvents {
    ...
    public static void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
        if (event.getEntity() instanceof ServerPlayer player) {
            ...
            // Sync TesterModality and require deterministic client/server compatibility handshake.
            try {
                PacketDistributor.sendToPlayer(player, new TesterModalitySyncPayload(TesterModality.isEnabled()));
                LOGGER.debug("[DevMod] Synced TesterModality to {}", player.getName().getString());
            } catch (Exception e) {
                LOGGER.warn("[DevMod] Failed to sync TesterModality to {}: {}",
                    player.getName().getString(), e.getMessage());
            }

            // Grant daily login season pass XP
            try {
                com.devmod.endurance.season.SeasonPassXPListener.INSTANCE.onPlayerLogin(player);
            } catch (Exception e) {
                LOGGER.warn("[DevMod] Failed to grant daily login XP to {}: {}",
                    player.getName().getString(), e.getMessage());
            }
            ...
            // Sync DuckDB-dependent tester features only when tester modality is enabled.
            if (testerModulesEnabled && DuckDBBootstrap.isAvailable()) {
            ...
            } else if (!testerModulesEnabled) {
                LOGGER.debug("[DevMod] TesterModality disabled: skipping tester-only login sync for {}",
