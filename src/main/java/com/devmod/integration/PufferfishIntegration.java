...
/**
 * Integration for Pufferfish Attributes mod.
 * Maps DevMod attributes to Pufferfish equivalents when available.
 */
public final class PufferfishIntegration {
    ...
    /**
     * @deprecated Use {@link #isAvailable()} instead for consistency with other integrations.
     */
    @Deprecated
    ...
    public static Attribute map(DeferredHolder<Attribute, Attribute> attribute) {
        ...
        if (mapped != null) {
            DevMod.LOGGER.debug("[PufferfishIntegration] Mapped {} to {}", incoming, targetId);
            return mapped;
        }

        DevMod.LOGGER.debug("[PufferfishIntegration] Missing {} in {}", targetId, MODID);
        return attribute.isBound() ? attribute.get() : null;
    }
