...
public class NotificationSoundManager {
    ...
    // ============================================================================
    // SINGLETON CLIENT PREFERENCES (for sound-only access)
    // ============================================================================

    /**
     * Client-side preferences singleton for sound manager.
     * Provides default implementations if main preferences not available.
     */
    @OnlyIn(Dist.CLIENT)
    public static class ClientNotificationPreferences {
        public static final ClientNotificationPreferences INSTANCE = new ClientNotificationPreferences();

        private boolean globalMute = false;
        private float masterVolume = 1.0f;
        private final Map<NotificationCategory, Float> categoryVolumes = new HashMap<>();

        private ClientNotificationPreferences() {
            // Default all categories to full volume
            for (NotificationCategory cat : NotificationCategory.values()) {
                categoryVolumes.put(cat, 1.0f);
            }
        }
        ...
        public boolean isSoundEnabled(NotificationCategory category) {
            ...
            Float vol = categoryVolumes.get(category);
            return vol == null || vol > 0.01f;
        }

        public float getSoundVolume(NotificationCategory category) {
            if (globalMute) return 0f;
            Float catVol = categoryVolumes.getOrDefault(category, 1.0f);
            return masterVolume * catVol;
        }
        ...
        public void setCategoryVolume(NotificationCategory category, float volume) {
