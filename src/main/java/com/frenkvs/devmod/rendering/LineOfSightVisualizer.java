...
public class LineOfSightVisualizer {
    ...
    public void render(@Nonnull PoseStack poseStack, @Nonnull MultiBufferSource buffer, @Nonnull Vec3 cameraPos) {
        ...
        for (Entity entity : mc.level.getEntities(mc.player, searchBox)) {
            ...
            // Render view cone
            renderViewCone(lineConsumer, matrix, pose, mob);

            // Render LoS to player
            renderLoSToPlayer(lineConsumer, matrix, pose, mob, playerPos, mc);

            // Render obstacles in view
            renderBlockingObstacles(lineConsumer, matrix, pose, mob, mc);
        }
    ...
    private void renderViewCone(@Nonnull VertexConsumer consumer, @Nonnull Matrix4f matrix,
        ...
        // Cone base center
        Vec3 baseCenter = eyePos.add(lookVec.scale(coneLength));
        ...
        for (int i = 0; i < segments; i++) {
            ...
            Vec3 offset = right.scale(Math.cos(angle) * coneRadius)
                    .add(actualUp.scale(Math.sin(angle) * coneRadius));
            Vec3 point = baseCenter.add(offset);
        ...
        Vec3 lookEnd = eyePos.add(lookVec.scale(coneLength));
        line(consumer, matrix, pose, tipX, tipY, tipZ,
    ...
    private void renderLoSToPlayer(@Nonnull VertexConsumer consumer, @Nonnull Matrix4f matrix,
        ...
        if (hasLoS && inFOV) {
            ...
            // Add label
            Vec3 midPoint = mobEye.add(playerEye).scale(0.5);
            ...
            DebugRenderer.INSTANCE.addLabel(mobEye.add(toPlayer.scale(3)), "OUT OF FOV", 0xFFFFFF00, 50);
    ...
    private void renderBlockingObstacles(@Nonnull VertexConsumer consumer, @Nonnull Matrix4f matrix,
        ...
        for (int i = -rayCount / 2; i <= rayCount / 2; i++) {
            for (int j = -rayCount / 2; j <= rayCount / 2; j++) {
                ...
                Vec3 rayDir = lookVec
                        .add(right.scale(Math.sin(yawOffset)))
                        .add(actualUp.scale(Math.sin(pitchOffset)))
                        .normalize();

                Vec3 rayEnd = eyePos.add(rayDir.scale(MOB_VIEW_DISTANCE));
