...
Replaces problematic paths with underscore-padded paths of same length.
"""
import sys
import os
...
def patch_efkefc(input_path, output_path):
    ...
    # Old path pattern (UTF-16LE): ../../TestData/Effects/Textures/Particle03.png (46 chars)
    old_path1 = '../../TestData/Effects/Textures/Particle03.png'.encode('utf-16-le')
    # New path: _______________________Textures/Particle03.png (46 chars)
    # 23 underscores + Textures/ + Particle03.png = 23 + 9 + 14 = 46 chars
    new_path1 = '_______________________Textures/Particle03.png'.encode('utf-16-le')

    # Verify same length
    assert len(old_path1) == len(new_path1), f"Length mismatch: {len(old_path1)} vs {len(new_path1)}"
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: patch_efkefc_v3.py <input.efkefc> [output.efkefc]")
        sys.exit(1)
