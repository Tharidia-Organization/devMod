...
### CRITICAL
- [x] ~~Replace chat fallback with a toast/notification overlay, queue WelcomeScreen for a safe retry, and remove the chat message path in `src/main/java/com/devmod/client/events/ClientModEvents.java`.~~ **FIXED**: Welcome fallback now uses `NotificationService`/`UnifiedToastOverlay`; `WelcomeToastOverlay` removed in orphanage cleanup.
...
- [x] ~~Define and apply a consistent ESC behavior policy across screens (including QuestExitConfirmScreen), with an "unsaved changes" indicator where relevant.~~ **REMOVED**: `EscapeBehavior.java` risultava legacy/unused ed e` stata rimossa in orphanage cleanup; ESC policy resta per‑screen (nessun helper condiviso).
...
- [x] ~~Add baseline accessibility features: keyboard navigation, high-contrast mode, and narration hooks for primary screens.~~ **PARTIAL**: High-contrast theme and settings remain; `FocusManager` helper removed in orphanage cleanup, quindi la navigazione keyboard e` per‑screen (nessun manager condiviso).
...
- [x] ~~Modularize `src/main/java/com/devmod/client/ui/editor/ItemEditorScreen.java` into smaller components and add targeted unit tests.~~ **REMOVED**: i componenti legacy (`HistoryPanel`, `ItemEditorDataOps`) sono stati rimossi in orphanage cleanup; la schermata resta auto‑contenuta (modularizzazione solo interna).
- [x] ~~Add loading indicators to data-fetching screens, with timeout and retry affordances.~~ **REMOVED**: `LoadingIndicator.java` era legacy/unused ed e` stato rimosso; resta `InstanceLoadingOverlay.java` per i flussi modali.
