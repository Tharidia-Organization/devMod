...
Questa sezione è ora **implementata**: overlay modale centrato con visualizzazione ricetta, selezione multi-ricetta, lista ingredienti scrollabile e analisi valore. Trigger tramite bottone “Recipe” nel footer (o tab dedicata).
...
Mostra la griglia di crafting 3x3 con gli ingredienti posizionati. Se esistono più ricette compatibili, l’overlay mostra controlli ←/→ (click o tastiera) per ciclare tra le ricette candidate; di default viene scelta la ricetta con valore totale più alto.
...
Calcola un valore numerico basato sulla rarità degli ingredienti; la lista ingredienti è scrollabile con scissor se eccede l’area utile e mostra “+more” tramite scrollbar (mouse wheel).
...
| UI | Overlay modale con chiusura via ESC/click fuori, frecce selezione ricetta, lista ingredienti scrollabile |
