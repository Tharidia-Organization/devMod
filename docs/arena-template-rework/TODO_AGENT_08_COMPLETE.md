...
| File | Description |
|------|-------------|
| `src/main/java/com/frenkvs/devmod/arena/cleanup/ArenaCleanupExecutor.java` | 4-phase cleanup executor (entities -> blockEntities -> scheduledTicks -> blocks) |
| `src/main/java/com/frenkvs/devmod/arena/cleanup/CleanupResult.java` | Result record with counters for each phase and warnings |
| `src/main/java/com/frenkvs/devmod/arena/cleanup/CleanupPhase.java` | Enum defining cleanup phases with order numbers |
| `src/main/java/com/frenkvs/devmod/arena/cleanup/CleanupVerification.java` | Post-cleanup verification with per-phase status |

### MSPT Monitor (DD38)

| File | Description |
|------|-------------|
| `src/main/java/com/frenkvs/devmod/arena/monitor/MsptMonitor.java` | Sliding window monitor (100 samples, 5 sec), baseline capture with median |
| `src/main/java/com/frenkvs/devmod/arena/monitor/MsptSample.java` | Sample record with confidence score and backpressure decision |

### Progress Overlay (DD39)

| File | Description |
|------|-------------|
| `src/main/java/com/frenkvs/devmod/arena/ui/BuildProgressOverlay.java` | Server-side overlay with 4Hz rate limit and 1% delta threshold |
| `src/main/java/com/frenkvs/devmod/arena/ui/BuildProgressHud.java` | Client-side HUD renderer with animated progress bar |
| `src/main/java/com/frenkvs/devmod/arena/network/BuildProgressPacket.java` | 28-byte network packet with phase, progress, and flags |

### Testing (DD40-41)

| File | Description |
|------|-------------|
| `src/test/java/com/frenkvs/devmod/arena/ArenaEdgeCaseTests.java` | Edge case tests with fixed seed (failure mid-build, chunk timeout, malformed template, concurrent parties) |
| `src/test/java/com/frenkvs/devmod/arena/MinecraftMockExtension.java` | JUnit 5 extension for mocking MC dependencies |
| `src/test/java/com/frenkvs/devmod/arena/CleanupUnitTests.java` | Unit tests for cleanup components |
| `src/test/java/com/frenkvs/devmod/arena/MsptMonitorUnitTests.java` | Unit tests for MSPT monitor |
| `src/test/java/com/frenkvs/devmod/arena/BuildProgressUnitTests.java` | Unit tests for progress overlay |
...
| File | Description |
|------|-------------|
| `src/main/java/com/frenkvs/devmod/arena/ArenaManager.java` | Manager with @Deprecated `createArena()` and runtime telemetry |
...
- 28-byte packet with: arenaId, phase, progress, blocksPlaced, totalBlocks, estimatedMs, flags
...
| Test Class | Tests |
|------------|-------|
| `CleanupUnitTests` | 15 tests covering CleanupResult, CleanupPhase, CleanupVerification |
| `MsptMonitorUnitTests` | 18 tests covering MsptSample and MsptMonitor |
| `BuildProgressUnitTests` | 20 tests covering packets and overlay |
| `ArenaEdgeCaseTests` | 15 edge case tests with fixed seed |
...
3. Minecraft dependencies are properly handled with mocking extension
